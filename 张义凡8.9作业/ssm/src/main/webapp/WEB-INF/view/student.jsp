...
<body class="gray-bg">
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStudent.htm">
						<input type="hidden" name="stid" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>学生姓名：</td>
								<td><input type="text" id="stname" name="stname" class="form-control" aria-required="true" required/></td>
								<td>&nbsp;&nbsp;</td>
								<td>学生生日：</td>
								<td><input type="date" id="stbirthday" name="stbirthday" class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>学生年龄：</td>
								<td><input type="text" id="stage" name="stage" class="form-control" aria-required="true" required/></td>
								<td>&nbsp;&nbsp;</td>
								<td>学生性别：</td>
								<td><input type="radio" value="男" name="stsex"/>男
								<input type="radio" value="女" name="stsex"/>女</td>
							</tr>
							<tr>
								<td>学校：</td>
								<td colspan="4">
									<select class="form-control" name="scid" id = "scid" aria-required="true" required>
										<option value="0">---请选择---</option>
										<c:forEach items="${sclist }" var="r">
										 	<option value="${r.sid }">${r.sname }</option>
										</c:forEach>
				                	</select>
								</td>
							</tr>
	...
	function initEditUser(getSelection){
		$('#hidden_txt_userid').val(getSelection.stid);//隐藏ID
		$('#stname').val(getSelection.stname);//学生姓名
		$('#stbirthday').val(new Date(getSelection.stbirthday).Format('yyyy-MM-dd'))//学生生日
		$('#stage').val(getSelection.stage);//学生年龄
		$('input[value="'+getSelection.stsex+'"]').prop("checked",true)//性别选中
		$('#scid').val(getSelection.scid);//学校ID
	}
